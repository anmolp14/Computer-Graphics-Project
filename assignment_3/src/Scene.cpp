...
vec3 Scene::trace(const Ray& _ray, int _depth)
{
    ...
    if (alpha != 0) {
        ...
        Ray reflected_ray(point + 5000*std::numeric_limits<double>::epsilon()*l , l);
...
vec3 Scene::lighting(const vec3& _point, const vec3& _normal, const vec3& _view, const Material& _material)
{
    ...
    for(int i=0; i<lights.size(); i++){
        ...
        Ray ray(_point + 5000*std::numeric_limits<double>::epsilon()*l, l);
        Object_ptr o=NULL;
        vec3 intersection_point, intersection_normal;
        double t;
        if ((!intersect(ray, o, intersection_point, intersection_normal, t)) || t>distance(_point, lights[i].position)) {
             vec3 r = reflect(-l, _normal);
             color += ( dot(_normal, l) < 0 ? vec3(0,0,0) : lights[i].color * _material.diffuse * dot(_normal, l) +
                      ((dot(r, _view)< 0 && dot(_normal, l) < 0) ?  vec3(0,0,0) : lights[i].color * _material.specular * std::pow(dot(r, _view),_material.shininess)) );
        }
    }
